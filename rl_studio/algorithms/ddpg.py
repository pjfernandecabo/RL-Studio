...
class Buffer:
    def __init__(self, num_states, num_actions, state_space, action_space, buffer_capacity=100000, batch_size=64):
    ...
    def record(self, obs_tuple):
        ...
        if self.state_space == 'image': #with image always will be continuous space
            self.state_buffer[index] = np.reshape(obs_tuple[0], obs_tuple[0].shape[0]* obs_tuple[0].shape[1] * obs_tuple[0].shape[2])
            self.action_buffer[index] = [obs_tuple[1][0][i] for i in range(len(obs_tuple[1][0]))]
            self.next_state_buffer[index] = np.reshape(obs_tuple[3], obs_tuple[3].shape[0] * obs_tuple[3].shape[1] * obs_tuple[3].shape[2])
    ...
    @tf.function
    def update(self, actor_critic, gamma, state_batch, action_batch, reward_batch, next_state_batch):

        with tf.GradientTape() as tape:
            target_actions = actor_critic.target_actor(next_state_batch, training=True)
            y = reward_batch + gamma * actor_critic.target_critic([next_state_batch, target_actions], training=True)

            if self.action_space == 'continuous':
                action_batch_v = action_batch[:,0]
                action_batch_w = action_batch[:,1]
                critic_value = actor_critic.critic_model([state_batch, action_batch_v, action_batch_w], training=True)
                
            else:
                critic_value = actor_critic.critic_model([state_batch, action_batch], training=True)

            critic_loss = tf.math.reduce_mean(tf.math.square(y - critic_value))

        critic_grad = tape.gradient(critic_loss, actor_critic.critic_model.trainable_variables)
        actor_critic.critic_optimizer.apply_gradients(zip(critic_grad, actor_critic.critic_model.trainable_variables))

        with tf.GradientTape() as tape:
            actions = actor_critic.actor_model(state_batch, training=True)
            critic_value = actor_critic.critic_model([state_batch, actions], training=True)
            # Used `-value` as we want to maximize the value given
            # by the critic for our actions
            actor_loss = -tf.math.reduce_mean(critic_value)

        actor_grad = tape.gradient(actor_loss, actor_critic.actor_model.trainable_variables)
        actor_critic.actor_optimizer.apply_gradients(zip(actor_grad, actor_critic.actor_model.trainable_variables))
    ...
    def learn(self, actor_critic, gamma):
        ...
        #ic(action_batch.shape)
        #ic(reward_batch.shape)
        #ic(next_state_batch.shape)

        if self.state_space == 'image':
            state_batch_no_flatten = tf.convert_to_tensor(np.reshape(self.state_buffer[batch_indices], [self.batch_size, self.num_states_dims[0],self.num_states_dims[1],self.num_states_dims[2]]))
            next_state_batch_no_flatten = tf.convert_to_tensor(np.reshape(self.next_state_buffer[batch_indices], [self.batch_size, self.num_states_dims[0],self.num_states_dims[1],self.num_states_dims[2]]))
            self.update(actor_critic, gamma, state_batch_no_flatten, action_batch, reward_batch, next_state_batch_no_flatten)

        else:
            self.update(actor_critic, gamma, state_batch, action_batch, reward_batch, next_state_batch)
...
class DDPGAgent():
    def __init__(self, config, action_space_size, observation_space_values, outdir):
        ...
        if config['state_space'] == 'image':
            self.OBSERVATION_SPACE_VALUES_FLATTEN = observation_space_values[0] * observation_space_values[1] * observation_space_values[2]   
        ...
        elif config['action_space'] != 'continuous' and config['state_space'] != 'image':
            self.actor_model = self.get_actor_model_simplified_perception_discrete_actions()
            self.critic_model = self.get_critic_model_simplified_perception_discrete_actions()
            # Actor Target model this is what we .predict against every step
            self.target_actor = self.get_actor_model_simplified_perception_discrete_actions()
            self.target_actor.set_weights(self.actor_model.get_weights())
            # Critic Target model this is what we .predict against every step
            self.target_critic = self.get_critic_model_simplified_perception_discrete_actions()
            self.target_critic.set_weights(self.critic_model.get_weights())

        elif config['action_space'] == 'continuous' and config['state_space'] == 'image':
            self.actor_model = self.get_actor_model_image_continuous_actions()
            self.critic_model = self.get_critic_model_image_continuous_actions()
            # Actor Target model this is what we .predict against every step
            self.target_actor = self.get_actor_model_image_continuous_actions()
            self.target_actor.set_weights(self.actor_model.get_weights())
            # Critic Target model this is what we .predict against every step
            self.target_critic = self.get_critic_model_image_continuous_actions()
            self.target_critic.set_weights(self.critic_model.get_weights())
    ...
    def get_actor_model_image_continuous_actions(self):
        ...
        model = Model(
            inputs=inputs,
            outputs=[v_Branch, w_Branch],
            name="continuous_two_actions")
